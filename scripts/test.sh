...
{
	OSS_REV=1.1.1a
	OSS_PKG=openssl-$OSS_REV
	OSS_DIR=$TRAVIS_BUILD_DIR/../$OSS_PKG
	OSS_SRC=https://www.openssl.org/source/$OSS_PKG.tar.gz
	OSS_INS=/usr/local/ssl
	OSS_OPT="shared no-tls no-dtls no-ssl3 no-zlib no-comp no-idea no-psk no-srp
			 no-stdio no-tests enable-rfc3779 enable-ec_nistp_64_gcc_128"

	if test -d "$OSS_DIR"; then
		return
	fi

	echo "$ build_openssl()"

	curl -L $OSS_SRC | tar xz -C $TRAVIS_BUILD_DIR/.. &&
	cd $OSS_DIR &&
	./config --prefix=$OSS_INS --openssldir=$OSS_INS $OSS_OPT &&
	make -j4 >/dev/null &&
	sudo make install_sw >/dev/null &&
	echo $OSS_INS/lib | sudo tee /etc/ld.so.conf.d/openssl-$OS_REV.conf >/dev/null &&
	sudo ldconfig || exit $?
	cd -
}
